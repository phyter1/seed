...
import { MLXOpenAICompatibleProviderAdapter } from "./mlx";

describe("MLXOpenAICompatibleProviderAdapter", () => {
  test("definition has local locality and local tier", () => {
    const adapter = new MLXOpenAICompatibleProviderAdapter();
    expect(adapter.id).toBe("mlx_openai_compatible");
    expect(adapter.displayName).toBe("MLX (OpenAI-compatible)");
